...
@section('title', 'Admin - Razen Project | Client')
...
@section('content')
    ...
    <div class="page-title-container">
        ...
        <div class="col-12 col-md-7">
            <h1 class="mb-0 pb-0 display-4" id="title"> Client</h1>
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    <li class="breadcrumb-item"><a href="{{ route('razen-project.admin.dashboard.index') }}">Dashboard</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('razen-project.admin.client.index') }}">Client</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <div class="row mb-3">
        <div class="col-6" style="text-align:left">
            <a href="{{ route('beranda') }}#clients" class="btn btn-icon waves-effect waves-light btn-secondary" target="blank"><i class="fas fa-pager"></i> Preview</a>
        </div>
    ...
    <div class="data-table-rows slim">
        ...
        <div class="data-table-responsive-wrapper">
            <table id="client_table" class="data-table w-100">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Nama</th>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="client_form" method="POST" enctype="multipart/form-data">
                        @csrf
...
@section('js')
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#client_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('razen-project.admin.client.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#client_form').on('submit', function(e){
            ...
            {
                $.ajax({
                    url: "{{ route('razen-project.admin.client.store') }}",
                    method: "POST",
            ...
            {
                $.ajax({
                    url: "{{ route('razen-project.admin.client.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            $.ajax({
                url: "{{ url('/razen-project/client/detail/') }}" + '/' + id,
                dataType: "json",
                success: function(data)
                {
                    $('#detail-title').text('Detail Data');
                    $('#detail_nama').val(data.result.nama);
                    $('#detail_foto').attr('src', "{{ asset('images/razen-project/foto-client') }}" + '/' + data.result.foto);
                    $('#detail').modal('show');
                }
            });
        });
        ...
        $(document).on('click', '.edit', function(){
            ...
            $('#form_result').html('');
            $.ajax({
                url: "{{ url('/razen-project/client/edit') }}"+'/'+id,
                dataType: "json",
                success: function(data)
                {
                    $('#nama').val(data.result.nama);
                    var lokasi_foto = "{{ asset('images/razen-project/foto-client') }}"+'/'+data.result.foto;
        ...
        $('#ok_button').click(function(){
            $.ajax({
                url: "{{ url('/razen-project/client/destroy') }}"+'/'+user_id,
                beforeSend: function(){
