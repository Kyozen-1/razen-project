...
@section('title', 'Admin - Razen Project | Layanan')
        ...
        <div class="col-12 col-md-7">
            ...
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('razen-project.admin.layanan.index') }}">Layanan</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#layanan_table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('razen-project.admin.layanan.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#hero_slider_form').on('submit', function(e){
            ...
            {
                $.ajax({
                    url: "{{ route('razen-project.admin.layanan.store') }}",
                    method: "POST",
            ...
            {
                $.ajax({
                    url: "{{ route('razen-project.admin.layanan.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            $.ajax({
                url: "{{ url('/razen-project/layanan/detail/') }}" + '/' + id,
                dataType: "json",
                success: function(data)
                {
                    ...
                    $('#detail_gambar').attr('src', "{{ asset('images/razen-project/gambar-layanan') }}" + '/' + data.result.gambar);
                    $('#detail').modal('show');
                }
            });
        });
        ...
        $(document).on('click', '.edit', function(){
            ...
            $('#form_result').html('');
            $.ajax({
                url: "{{ url('/razen-project/layanan/edit') }}"+'/'+id,
                dataType: "json",
                success: function(data)
                {
                    $('#judul').val(data.result.judul);
                    $('#deskripsi').val(data.result.deskripsi);
                    var lokasi_gambar = "{{ asset('images/razen-project/gambar-layanan') }}"+'/'+data.result.gambar;
        ...
        $('#ok_button').click(function(){
            $.ajax({
                url: "{{ url('/razen-project/layanan/destroy') }}"+'/'+user_id,
                beforeSend: function(){
