...
@section('title', 'Admin - Razen Project | Hero Slider')
...
@section('content')
    ...
    <div class="page-title-container">
        ...
        <div class="col-12 col-md-7">
            <h1 class="mb-0 pb-0 display-4" id="title"> Hero Slider</h1>
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    <li class="breadcrumb-item"><a href="{{ route('razen-project.admin.dashboard.index') }}">Dashboard</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('razen-project.admin.hero-slider.index') }}">Hero Slider</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <div class="row mb-3">
        <div class="col-6" style="text-align:left">
            <a href="{{ route('beranda') }}#hero-slider" class="btn btn-icon waves-effect waves-light btn-secondary" target="blank"><i class="fas fa-pager"></i> Preview</a>
        </div>
    ...
    <div class="data-table-rows slim">
        ...
        <div class="data-table-responsive-wrapper">
            <table id="hero_slider_table" class="data-table w-100">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Heading</th>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="hero_slider_form" method="POST" enctype="multipart/form-data">
                        @csrf
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group mb-3 position-relative">
                        <label for="detail_heading" class="control-label">Heading</label>
                        <input type="text" id="detail_heading" class="form-control" disabled>
                    </div>
                    <div class="form-group mb-3 position-relative">
                        <label for="detail_title" class="control-label">Title</label>
                        <input type="text" id="detail_title" class="form-control" disabled>
                    </div>
                    <div class="form-group mb-3 position-relative">
...
@section('js')
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#hero_slider_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('razen-project.admin.hero-slider.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#hero_slider_form').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ route('razen-project.admin.hero-slider.store') }}",
                    method: "POST",
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.success)
                        {
                            ...
                            $('#hero_slider_form')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#hero_slider_table').DataTable().ajax.reload();
                        }
            ...
            {
                $.ajax({
                    url: "{{ route('razen-project.admin.hero-slider.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            $.ajax({
                url: "{{ url('/razen-project/hero-slider/detail/') }}" + '/' + id,
                dataType: "json",
                success: function(data)
                {
                    $('#detail-title').text('Detail Data');
                    $('#detail_heading').val(data.result.heading);
                    $('#detail_title').val(data.result.title);
                    $('#detail_gambar').attr('src', "{{ asset('images/razen-project/gambar-slider') }}" + '/' + data.result.gambar);
                    $('#detail').modal('show');
                }
            });
        });
        ...
        $(document).on('click', '.edit', function(){
            ...
            $('#form_result').html('');
            $.ajax({
                url: "{{ url('/razen-project/hero-slider/edit') }}"+'/'+id,
                dataType: "json",
                success: function(data)
                {
                    $('#heading').val(data.result.heading);
                    $('#form_title').val(data.result.title);
                    var lokasi_gambar = "{{ asset('images/razen-project/gambar-slider') }}"+'/'+data.result.gambar;
        ...
        $('#ok_button').click(function(){
            $.ajax({
                url: "{{ url('/razen-project/hero-slider/destroy') }}"+'/'+user_id,
                beforeSend: function(){
