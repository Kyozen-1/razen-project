...
@section('title', 'Admin - Razen Project | Profil')
...
@section('content')
        ...
        <div class="col-12 col-md-7">
            ...
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('razen-project.admin.profil.index') }}">Profil</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <div class="card mb-3">
        <div class="card-body">
            <form action="{{ route('razen-project.admin.profil.store') }}" novalidate="novalidate" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row mb-3">
                ...
                <div class="row">
                    ...
                    <div class="col-12 col-md-4">
                        <input type="file" class="dropify" name="logo" data-height="300" data-allowed-file-extensions="png jpg jpeg webp" data-default-file="{{ asset('images/razen-project/logo/'.$profil->logo) }}" data-show-errors="true" required>
                    </div>
                </div>
            </form>
        </div>
    ...
    <div class="card">
        <div class="card-body">
            ...
            @if ($pivot_profil_razen_project_media_sosial['status'] == 'ada')
                <form id="edit_media_sosial_profil_form" novalidate="novalidate" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row mb-3">
                        @php
                            $a = 1;
                        @endphp
                        @foreach ($pivot_profil_razen_project_media_sosial['pivot'] as $item)
                            <div class="col-12 col-md-4 col-media-sosial-profil" id="{{$item->id}}">
                                <div class="border shadow p-3 mb-3">
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-2">
                                            <button class="close hapus-media-sosial-profil" type="button" aria-hidden="true" data-id="{{$item->id}}" title="Hapus Media Profil">x</button>
                                        </div>
                                    </div>
...
@section('js')
    ...
    <script>
        ...
        var ID_PIVOT_PROFIL_RAZEN_PROJECT_MEDIA_SOSIAL = [];

        $('.hapus-media-sosial-profil').click(function(){
            var id = $(this).attr('data-id');
            ID_PIVOT_PROFIL_RAZEN_PROJECT_MEDIA_SOSIAL.push(id);
        ...
        $('#edit_media_sosial_profil_form').submit(function(e){
            ...
            return new swal({
                ...
                {
                    $.ajax({
                        url: "{{ route('razen-project.admin.profil.edit-media-sosial-profil') }}",
                        method: "POST",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            id:ID_PIVOT_PROFIL_RAZEN_PROJECT_MEDIA_SOSIAL,
                        },
                        beforeSend: function()
                        {
                            return new swal({
                            ...
                            if(data.success)
                            {
                                new swal({
                                    ...
                                    title: data.success
                                        ...
                                        window.location.href = "{{ route('razen-project.admin.profil.index') }}";
                                });
                            }
                        }
                    });
        ...
        function dynamic_field_media_sosial(number_media_sosial)
        {
            ...
            html += '</div>';
            html += '</div>';
            html += '<input type="hidden" name="data_media_sosial['+index_media_sosial+'][profil_razen_project_id]" value="'+profil_id+'">';
        ...
        $('#tambah_media_sosial_profil_form').submit(function(e){
            e.preventDefault();
            $.ajax({
                url: "{{ route('razen-project.admin.profil.tambah-media-sosial-profil') }}",
                method: "POST",
