...
<script>
    const settings = new Settings({
        @if (Auth::guard('razen_project')->check())
            attributes: {
                color: "{{Auth::guard('razen_project')->user()->color_layout?Auth::guard('razen_project')->user()->color_layout:'light-blue'}}",
                navcolor: "{{Auth::guard('razen_project')->user()->nav_color?Auth::guard('razen_project')->user()->nav_color:'light'}}",
                behaviour: "{{Auth::guard('razen_project')->user()->behaviour?Auth::guard('razen_project')->user()->behaviour:'pinned'}}",
                layout: "{{Auth::guard('razen_project')->user()->layout?Auth::guard('razen_project')->user()->layout:'fluid'}}",
                radius: "{{Auth::guard('razen_project')->user()->radius?Auth::guard('razen_project')->user()->radius:'flat'}}",
                placement: "{{Auth::guard('razen_project')->user()->placement?Auth::guard('razen_project')->user()->placement:'vertical'}}",
            }
        @endif
    });
    $('.option').click(function(){
        ...
        $.ajax({
            url: "{{ route('razen-project.admin.dashboard.change') }}",
            method: 'POST',
            data: {
                "_token": "{{ csrf_token() }}",
