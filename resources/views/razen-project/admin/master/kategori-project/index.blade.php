...
@section('title', 'Admin - Razen Project | Master Kategori Project')
...
@section('content')
    ...
    <div class="page-title-container">
        ...
        <div class="col-12 col-md-7">
            <h1 class="mb-0 pb-0 display-4" id="title"> Master Kategori Project</h1>
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    <li class="breadcrumb-item"><a href="{{ route('razen-project.admin.dashboard.index') }}">Dashboard</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('razen-project.admin.master-kategori-project.index') }}">Master Kategori Project</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <div class="data-table-rows slim">
        ...
        <div class="data-table-responsive-wrapper">
            <table id="master_kategori_project_table" class="data-table w-100">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Nama</th>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="master_kategori_project_form" method="POST" enctype="multipart/form-data">
                        @csrf
...
@section('js')
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#master_kategori_project_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('razen-project.admin.master-kategori-project.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#create').click(function(){
            $('#master_kategori_project_form')[0].reset();
        ...
        $('#master_kategori_project_form').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ route('razen-project.admin.master-kategori-project.store') }}",
                    method: "POST",
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.success)
                        {
                            ...
                            $('#master_kategori_project_form')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#master_kategori_project_table').DataTable().ajax.reload();
                        }
            ...
            {
                $.ajax({
                    url: "{{ route('razen-project.admin.master-kategori-project.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            $.ajax({
                url: "{{ url('/razen-project/master-kategori-project/detail/') }}" + '/' + id,
                dataType: "json",
                success: function(data)
                {
        ...
        $(document).on('click', '.edit', function(){
            ...
            $('#form_result').html('');
            $.ajax({
                url: "{{ url('/razen-project/master-kategori-project/edit') }}"+'/'+id,
                dataType: "json",
                success: function(data)
                {
        ...
        $('#ok_button').click(function(){
            $.ajax({
                url: "{{ url('/razen-project/master-kategori-project/destroy') }}"+'/'+user_id,
                beforeSend: function(){
